...
describe('Address encoder', () => {
  testVectors.forEach((testCase) => {
    it(`should encode ${testCase.public.toString('hex')} as cashaddr`, () => {
      const address = addressEncoder.encodeCashaddress('bitcoincash', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.address);
    });

    it(`should encode ${testCase.public.toString('hex')} as SLP address`, () => {
      const address = addressEncoder.encodeCashaddress('simpleledger', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.slp);
    });

    it(`should encode ${testCase.public.toString('hex')} as legacy address`, () => {
      const address = addressEncoder.encodeLegacy('mainnet', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.legacy);
    });

    it(`should encode ${testCase.public.toString('hex')} as testnet cashaddr`, () => {
      const address = addressEncoder.encodeCashaddress('bchtest', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.addressTestnet);
    });

    it(`should encode ${testCase.public.toString('hex')} as testnet SLP address`, () => {
      const address = addressEncoder.encodeCashaddress('slptest', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.slpTestnet);
    });

    it(`should encode ${testCase.public.toString('hex')} as testnet legacy address`, () => {
      const address = addressEncoder.encodeLegacy('testnet', 'P2PKH', testCase.hash160);
      expect(address).toBe(testCase.legacyTestnet);
    });

    it(`should decode ${testCase.address}`, () => {
      const decoded = addressEncoder.decodeCashaddress(testCase.address);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('mainnet');
      expect(decoded.type).toBe('P2PKH');
    });

    it(`should decode ${testCase.slp}`, () => {
      const decoded = addressEncoder.decodeCashaddress(testCase.slp);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('mainnet');
      expect(decoded.type).toBe('P2PKH');
    });

    it(`should decode ${testCase.legacy}`, () => {
      const decoded = addressEncoder.decodeLegacy(testCase.legacy);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('mainnet');
      expect(decoded.type).toBe('P2PKH');
    });

    it(`should decode ${testCase.addressTestnet}`, () => {
      const decoded = addressEncoder.decodeCashaddress(testCase.addressTestnet);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('testnet');
      expect(decoded.type).toBe('P2PKH');
    });

    it(`should decode ${testCase.slpTestnet}`, () => {
      const decoded = addressEncoder.decodeCashaddress(testCase.slpTestnet);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('testnet');
      expect(decoded.type).toBe('P2PKH');
    });

    it(`should decode ${testCase.legacyTestnet}`, () => {
      const decoded = addressEncoder.decodeLegacy(testCase.legacyTestnet);
      expect(decoded.hash160).toStrictEqual(testCase.hash160);
      expect(decoded.network).toBe('testnet');
      expect(decoded.type).toBe('P2PKH');
    });
